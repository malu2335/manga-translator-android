...
class EmbeddedTextRenderer {
    ...
    private fun drawTextInRect(
        ...
        if (verticalLayoutEnabled) {
            ...
            drawVerticalTextInRect(canvas, converted, paddedRect, drawBackground)
        } else {
            val maxWidth = paddedRect.width().toInt().coerceAtLeast(1)
            val maxHeight = paddedRect.height().toInt().coerceAtLeast(1)
            var textSize = (paddedRect.height() / 3f).coerceIn(12f, 42f)
            ...
            val dx = paddedRect.left
            val dy = paddedRect.top + ((paddedRect.height() - layout.height) / 2f).coerceAtLeast(0f)
            drawHorizontalTextLayout(canvas, text, layout, dx, dy, paddedRect, drawBackground)
        }
    }
