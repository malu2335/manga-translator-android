...
- `BubbleTextScaling.kt`：文本缩放共享算法，提供布局拟合判定、路径缩放、水平文本二分搜索与缩放方案构建，三个渲染端点 (`BubbleRenderer`、`FloatingTranslationView`、`FloatingDetectionOverlayView`) 均调用该模块。
...
- 普通气泡框设置：普通气泡回缩、普通气泡不透明度、游离气泡回缩、游离气泡不透明度、最小字号、气泡放大开关、横向/竖向排版；作用范围是阅读页、条漫页、导出图片。
- 最小字号是普通翻译的文字下限。排版会先尝试求出适合当前气泡的字号；如果结果小于该值，则强制抬到该下限或按比例缩放（取决于气泡放大开关）。
- 这里的”游离气泡”仅指普通模式下由 `TextDetector` 补出的框，不包含用户手动新增气泡；手动新增气泡继续按普通气泡参数渲染。
- 悬浮窗气泡设置：大小外扩/内缩、不透明度、矩形/内接椭圆、横向/竖向排版、最小字号、气泡放大开关；作用范围仅限悬浮窗 overlay，各项参数与普通气泡框完全独立。
...
- 气泡框渲染配置：`normal_bubble_shrink_percent`、`normal_bubble_min_font_size_sp`、`expand_bubble_when_min_font_size`、`horizontal_text_layout`、`floating_bubble_size_adjust_percent`、`floating_bubble_opacity_percent`、`floating_bubble_shape`、`floating_bubble_horizontal_text`、`floating_bubble_min_font_size_sp`、`floating_expand_bubble_when_min_font_size`
