...
- 悬浮窗气泡框：仅用于悬浮窗 overlay，设置入口在 `SettingsFragment.kt` 的“悬浮窗气泡设置”，渲染主入口是 `FloatingDetectionOverlayView.kt`；文字最小字号与普通气泡框共用同一设置。
...
当前与气泡框渲染直接相关的设置分为两组：
- 普通气泡框设置：普通气泡回缩、普通气泡不透明度、游离气泡回缩、游离气泡不透明度、最小字号、横向/竖向排版；作用范围是阅读页、条漫页、导出图片。
- 最小字号是普通翻译与悬浮窗翻译共用的文字下限。排版会先尝试求出适合当前气泡的字号；如果结果小于该值，则强制抬到该下限。
- 这里的“游离气泡”仅指普通模式下由 `TextDetector` 补出的框，不包含用户手动新增气泡；手动新增气泡继续按普通气泡参数渲染。
- 悬浮窗气泡设置：大小外扩/内缩、不透明度、矩形/内接椭圆、横向/竖向排版；作用范围仅限悬浮窗 overlay，不再单独提供字号百分比设置。
...
- 气泡框渲染配置：`normal_bubble_shrink_percent`、`normal_bubble_min_font_size_sp`、`horizontal_text_layout`、`floating_bubble_size_adjust_percent`、`floating_bubble_opacity_percent`、`floating_bubble_shape`、`floating_bubble_horizontal_text`
- 兼容说明：`normal_bubble_font_scale_percent` 仅保留为旧版本配置迁移读取使用，新写入统一落到 `normal_bubble_min_font_size_sp`。
