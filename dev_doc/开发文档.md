...
| OCR 相关 | `app/src/main/java/com/manga/translate/OcrSharedTools.kt`、`OcrEngine.kt`、`JapanesePpOcr.kt`、`MangaOcr.kt`、`EnglishOcr.kt`、`KoreanOcr.kt` |
...
- `JapanesePpOcr.kt`、`MangaOcr.kt`、`EnglishOcr.kt`、`KoreanOcr.kt`：本地 OCR 实现。日文默认走 `JapanesePpOcr`，可在 OCR 设置中切换到 `MangaOcr`。
...
- 新增 OCR 入口时，优先复用共享 OCR 依赖，不要在业务类里重复持有 `JapanesePpOcr`、`MangaOcr`、`EnglishOcr`、`KoreanOcr`、`EnglishLineDetector` 实例。
